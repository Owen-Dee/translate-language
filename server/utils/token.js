...
async function updateTKK(opts, gotopts) {
    opts = opts || {tld: 'com'};
    ...
    if (Number(window.TKK.split('.')[0]) === now) {
        ...
        try {
            const res = await got('https://translate.google.' + opts.tld, gotopts);
...
async function get(text, opts, gotopts) {
    try {
        await updateTKK(opts, gotopts);
        let tk = sM(text);
        tk = tk.replace('&tk=', '');
